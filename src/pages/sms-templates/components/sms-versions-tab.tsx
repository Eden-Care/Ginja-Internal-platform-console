...
import type { SmsVersionRow } from "@/features/sms-templates/types"
import {
  useSmsTemplateCompare,
  useSmsTemplateVersions,
} from "@/features/sms-templates/use-sms-templates"

const FIELD_LABEL: Record<string, string> = { message_text: "Message" }
const fieldLabel = (f: string) =>
  FIELD_LABEL[f] ??
  f.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())

/** The SMS editor's "Versions" tab — live history (with per-version status) +
   server-computed diff of the predecessor → selected version. */
export function SmsVersionsTab({
  templateId,
  readonly,
  onRollback,
}: {
  templateId?: number
  readonly: boolean
  onRollback: (v: SmsVersionRow) => void
}) {
  const versionsQuery = useSmsTemplateVersions(templateId)
  ...
  const compareQuery = useSmsTemplateCompare(templateId, from, to)
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <b className="text-[13.5px]">v{v.versionLabel}</b>
                        {v.status ? (
