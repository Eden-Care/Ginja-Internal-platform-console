...
import { useSmsTemplateActivity } from "@/features/sms-templates/use-sms-templates"
...
const ACTION_META: Record<string, { icon: React.ReactNode; tone: Tone }> = {
  ...
  ARCHIVED: { icon: <HistoryIcon />, tone: "warning" },
  RESTORED: { icon: <HistoryIcon />, tone: "info" },
  TEST_SENT: { icon: <SendIcon />, tone: "info" },
...
/** The SMS editor's "Audit log" tab — the live, attributed event feed. */
export function SmsAuditTab({ templateId }: { templateId?: number }) {
  const activityQuery = useSmsTemplateActivity(templateId)
