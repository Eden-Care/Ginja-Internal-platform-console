...
/** Maps a template's `used_by` to its filter label. */
const scopeLabel = (usedBy?: string) =>
  (usedBy ?? "").toUpperCase().includes("CONSOLE")
    ? "Internal console"
    : "Tenant platforms"

/** Card status badge — Archived / Published / Disabled / Draft.
   "Disabled" only applies to a *published* template that's been deactivated;
   an unpublished template is always "Draft" regardless of the active flag. */
function statusBadge(
  t: EmailTemplate,
  archived: boolean
): { tone: "success" | "warning" | "neutral"; label: string } {
  if (archived) return { tone: "neutral", label: "Archived" }
...
export function EmailTemplatesPage() {
  ...
  const tplQuery = useInfiniteEmailTemplates({ archived: showArchived })
  const templates = React.useMemo(
    () => tplQuery.data?.pages.flatMap((p) => p.items) ?? [],
    [tplQuery.data]
  )

  // A tiny count-only query for the "Archived (N)" badge while viewing active.
  const archivedCountQuery = useEmailTemplates({
    page: 0,
    size: 1,
    archived: true,
  })
  ...
  const activeMut = useSetEmailTemplateActive()
  const archiveMut = useSetEmailTemplateArchived()
  const dupMut = useDuplicateEmailTemplate()
  const publishMut = usePublishEmailTemplate()

  const toggleArchivedView = () => setShowArchived((s) => !s)
  ...
  const setArchived = (t: EmailTemplate, archived: boolean) => {
    if (t.templateId == null) return
    archiveMut.mutate(
      ...
      {tplQuery.isLoading ? (
        ...
        <div className="grid [grid-template-columns:repeat(auto-fill,minmax(300px,1fr))] gap-3.5">
          {rows.map((t) => {
            ...
            const badge = statusBadge(t, isArchived)
              ...
              >
                ...
                <div className="mt-auto flex items-center gap-2 pt-1">
                  ...
                  <Tagpill className="ml-auto text-[10.5px]">
                    ...
                    {isConsole ? "Internal console" : "Tenant"}
                  </Tagpill>
                </div>
              </div>
            )
