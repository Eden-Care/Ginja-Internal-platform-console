/**
 * Icons rendered from the hi-fi reference's exact lucide SVG paths
 * (`Ginja Console-v3.html`), the same way the hi-fi does (24-viewBox, 1.75
 * stroke, round caps). Used instead of `lucide-react` components where the
 * installed lucide version draws a glyph differently than the reference was
 * built against (e.g. the "Published" check is `CircleCheckBig` here, not
 * `CheckCircle2`; `pencil` is the older two-stroke pencil).
 *
 * Covers the module-registry + email/SMS template screens. Unknown names fall
 * back to the building glyph.
 */
const PATHS: Record<string, string> = {
  ...
  providers:
    '<path d="M16 7a4 4 0 1 0-8 0"/><circle cx="12" cy="7" r="0.01"/><path d="M12 11v10"/><path d="M5 21h14"/><circle cx="5" cy="14" r="3"/><circle cx="19" cy="14" r="3"/><path d="M8 14h8"/>',
  ...
  analytics:
    '<path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="M7 16l4-6 4 4 5-7"/>',
  reinsurance:
  ...
  send: '<path d="M14.54 21.69a.5.5 0 0 0 .94-.03l6.5-19a.5.5 0 0 0-.64-.63l-19 6.5a.5.5 0 0 0-.02.93l7.93 3.18a2 2 0 0 1 1.1 1.1z"/><path d="m21.85 2.15-10.94 10.94"/>',
  smartphone:
  ...
  sparkles:
    '<path d="M9.94 14.34A2 2 0 0 0 8.66 13.06L3 11l5.66-2.06a2 2 0 0 0 1.28-1.28L12 2l2.06 5.66a2 2 0 0 0 1.28 1.28L21 11l-5.66 2.06a2 2 0 0 0-1.28 1.28L12 20Z"/><path d="M22 5h-4"/><path d="M20 3v4"/><path d="M5 19v2"/><path d="M4 20h2"/>',
  ...
  paperclip:
    '<path d="M13.234 20.252 21 12.3a4.25 4.25 0 0 0-6.01-6.01l-9.193 9.193a2.833 2.833 0 1 0 4.007 4.007l8.486-8.486"/>',
  upload:
  ...
  // service-provider (Claim Clean-up) glyphs
  hospital:
  ...
  // provider⇆insurer workspace + contracts/rule-extraction glyphs
  quote:
...
/** Renders a hi-fi icon by name. Size/colour come from the container
 * (`[&>svg]:size-*` + `text-*`); falls back to the building glyph. */
export function HiIcon({
  ...
  return (
    <svg
      ...
      className={className}
      aria-hidden="true"
      dangerouslySetInnerHTML={{ __html: PATHS[name] ?? PATHS.building }}
    />
  )
}
